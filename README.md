...
When signed Android release, installed Android tour, backup restore, rollback rehearsal, provider-backed email, alert-routing, or final live stakeholder walkthrough proof already exists, pass the generated JSON paths with `-AndroidReleaseManifestPath`, `-InstalledAndroidTourReportPath`, `-BackupRestoreManifestPath`, `-RollbackManifestPath`, `-EmailProviderProofManifestPath`, `-AlertRoutingManifestPath`, and `-LiveStakeholderWalkthroughManifestPath` so the deployment evidence manifest can attach their schemas and paths without storing secrets or logs. Android release attachments must point to an existing APK/AAB whose SHA-256, byte size, commit SHA, cleartext policy, and external-signing boundary match the sanitized release manifest and deployed commit; when the signed artifact is an APK, the installed Android tour fingerprint must match that APK. Backup/restore attachments must prove a matching backup hash, byte size, restore status, and secret-handling policy; rollback attachments must prove env/Compose preflight, rollback execution, secret-handling policy, and post-rollback monitoring against the same deployed frontend/API URLs. Proven SMTP/provider labels require a matching email-provider proof artifact for password recovery, access-request/account-ready, and notification email workflows with per-workflow `SENT` provider statuses; the final stakeholder walkthrough artifact must declare `proofMode=manual-live-review` so script-only tours cannot close the live review requirement; `email-disabled-by-policy` is the only provider label that can close email evidence without that artifact. The manifest still records `productionClaim=false` until a deliberate cutover decision is made after reviewing remaining evidence.
