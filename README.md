...
When signed Android release, installed Android tour, backup restore, rollback rehearsal, or alert-routing proof already exists, pass the generated JSON paths with `-AndroidReleaseManifestPath`, `-InstalledAndroidTourReportPath`, `-BackupRestoreManifestPath`, `-RollbackManifestPath`, and `-AlertRoutingManifestPath` so the deployment evidence manifest can attach their schemas and paths without storing secrets or logs. The manifest still records `productionClaim=false` until the remaining operations and live stakeholder proof are complete.
