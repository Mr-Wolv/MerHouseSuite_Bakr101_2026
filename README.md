...
The default preflight parses scripts, audits the deployment env template, validates the VPS Compose shape, frontend nginx same-origin `/api` proxy, and public reverse-proxy shape, checks Android release configuration, proves deployed-evidence attachment rules, exercises the cutover-readiness guard with fixtures, runs markdown/public-readiness checks, and rebuilds the frontend for performance budgets. It intentionally leaves live load, signed Android artifact proof, and deployed cutover evidence to explicit HTTPS target-backed commands.
...
.\scripts\deploy\bootstrap-owner.ps1 -EnvFile ".secrets/deploy/env.production" -OwnerEmail "<owner-email>" -OwnerPassword "<private-owner-password>" -ConfirmBootstrap
...
V17 private deployment work adds a selected Neon + Render + Vercel + GitHub Release lane, a fallback VPS + Docker Compose shape under `deploy/vps/`, deployment scripts under `scripts/deploy/`, private env auditing, deployed proof wrappers, HTTPS deployed monitoring samples with explicit local-rehearsal escape hatches, backup/restore drill manifests, SMTP-backed email delivery hooks with sanitized provider-proof artifacts, sanitized alert-routing and manual live-walkthrough proof artifacts, a read-plus-draft agent runtime interface with deterministic fallback metadata, load-smoke proof, Android release-shape proof, and signed internal Android release checks with sanitized release manifests. The first V17 deployment should stay simple: deploy and prove the existing product, use CI/CD plus deployed smoke/load/browser/mobile proof for bug hunting after launch, and leave only the agent as v1 prototype-grade read-plus-draft behavior with no operational mutations. Keep detailed deployment values, provider credentials, Android keystores, and proof logs private until the deployment is proven and intentionally published.

Deployment lanes are tracked in the activation doc: first Neon PostgreSQL plus Render backend plus Vercel frontend plus GitHub Release APK distribution; next Cloud Run if Render/Vercel constraints block rollout; then Oracle Cloud Always Free as a low-cost VPS-style fallback; then sponsored professional hosting when budget exists for managed infrastructure, database, monitoring, and operations.
