...
- `.\scripts\quality\deployment-readiness.ps1 -IncludeE2E -IncludeApiSmoke -SkipCompose` passed before `-NativeApiBaseUrl` was made explicit, using the native sync default `http://10.0.2.2:8080`; the current command should pass `-NativeApiBaseUrl "http://10.0.2.2:8080"` so the Android backend target is visible in the proof transcript.
- Mobile-native proof includes shared mobile shell metadata, app icon, online-first service worker, mobile shell navigation, Capacitor Android wrapper source, APK assembly, and a 100-record installed-APK route tour with APK fingerprint `34a1549b2b9370779f958839ede8a9d004d62a23cced53ad6c5dafedfb1aabd7`, device, timestamp, and checked-route provenance across public routes, owner, generated admin, support-admin, auditor, active merchant, empty merchant, active warehouse, empty warehouse, and discovered operational detail routes.
...
- `.\scripts\quality\cross-surface-tour-check.ps1 -WebReportPath ".\reports\v16.2-loop-163-frontend-full-tour.json" -NativeReportPath ".\reports\v16.2-loop-163-native-tour.json"` passed with the fresh browser report and fresh installed-APK report, browser target provenance, exact normalized role/path equality, APK fingerprint/device/timestamp/checked-route provenance, and no bad route records.
- `.\scripts\quality\deployment-readiness.ps1 -IncludeE2E -IncludeApiSmoke -SkipCompose -WebTourReportPath ".\reports\v16.2-loop-163-frontend-full-tour.json" -NativeTourReportPath ".\reports\v16.2-loop-163-native-tour.json"` passed the full report-aware deployment gate after the fresh browser and installed-APK tours, using the native sync default `http://10.0.2.2:8080`; the current equivalent command includes `-NativeApiBaseUrl "http://10.0.2.2:8080"`.
- `.\scripts\quality\performance-readiness.ps1 -IncludeApiSmoke -ApiBaseUrl "http://localhost:8080" -WebReportPath ".\reports\v16.2-loop-163-frontend-full-tour.json" -NativeReportPath ".\reports\v16.2-loop-163-native-tour.json" -OutputPath ".\reports\v16.2-loop-163-performance-readiness.json"` passed local bundle/performance budgets, required browser report provenance, required native APK/device/timestamp/checked-route provenance, required per-record web/native route timing, required per-record native screenshot timing, and 35.18-second timed API smoke under the 120-second budget; the full deployment gate then recorded 29.29-second timed API smoke in `reports/performance-readiness.json`. Production load, monitoring, autoscaling, and provider-backed delivery remain later activation work.
