...
& (Join-Path $PSScriptRoot "mobile-shell-check.ps1")
...
& (Join-Path $PSScriptRoot "native-mobile-check.ps1") -Sync -ApiBaseUrl $normalizedNativeApiBaseUrl

if ($hasWebTourReport -and $hasNativeTourReport) {
    Write-Host "Running cross-surface tour report proof..."
    & (Join-Path $PSScriptRoot "cross-surface-tour-check.ps1") -WebReportPath $resolvedWebTourReportPath -NativeReportPath $resolvedNativeTourReportPath
} else {
...
& (Join-Path $PSScriptRoot "performance-readiness.ps1") @performanceArgs
