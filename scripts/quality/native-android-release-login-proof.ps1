...
Invoke-Adb shell input tap $tapX $emailY | Out-Null
Start-Sleep -Milliseconds 500
Send-AdbText -Text $Email
Invoke-Adb shell input keyevent KEYCODE_TAB | Out-Null
Start-Sleep -Milliseconds 500
...
Invoke-Adb shell input tap $tapX $signInY | Out-Null
Start-Sleep -Seconds 20
$signedInScreenshot = Capture-Screenshot -Name "03-after-sign-in"

$windowDumpPath = "/sdcard/merhouse-window.xml"
$windowLocalPath = Join-Path $resolvedScreenshotDirectory "03-after-sign-in-window.xml"
Invoke-Adb shell uiautomator dump $windowDumpPath | Out-Null
Invoke-Adb pull $windowDumpPath $windowLocalPath | Out-Null
Invoke-Adb shell rm $windowDumpPath | Out-Null
$windowText = Get-Content -LiteralPath $windowLocalPath -Raw
