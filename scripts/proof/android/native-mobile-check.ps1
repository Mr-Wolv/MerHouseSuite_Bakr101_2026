...
if (-not [string]::IsNullOrWhiteSpace($FrontendUrl)) {
    ...
    if (-not $normalizedFrontendUrl.StartsWith("https://")) {
        throw "Android deployed frontend wrappers must use an https FrontendUrl."
    }
...
if ($normalizedFrontendUrl) {
    Write-Host "Native Android deployed frontend URL: $normalizedFrontendUrl"
}
...
if ($Sync) {
    Invoke-Step "Building web assets for native shell..." {
        ...
        try {
            ...
            $previousAndroidFrontendUrl = $env:MERHOUSE_ANDROID_FRONTEND_URL
            $env:VITE_API_BASE_URL = $normalizedApiBaseUrl
            if ($normalizedFrontendUrl) {
                $env:MERHOUSE_ANDROID_FRONTEND_URL = $normalizedFrontendUrl
            } else {
                Remove-Item Env:MERHOUSE_ANDROID_FRONTEND_URL -ErrorAction SilentlyContinue
            }
            npm run build
            ...
            if ($null -eq $previousAndroidFrontendUrl) {
                Remove-Item Env:MERHOUSE_ANDROID_FRONTEND_URL -ErrorAction SilentlyContinue
            } else {
                $env:MERHOUSE_ANDROID_FRONTEND_URL = $previousAndroidFrontendUrl
            }
            Pop-Location
        }
    }
    ...
    Invoke-Step "Checking native frontend API base..." {
        ...
        if ($normalizedFrontendUrl) {
            Write-Host "Native wrapper will load deployed frontend URL: $normalizedFrontendUrl"
        }
    }
