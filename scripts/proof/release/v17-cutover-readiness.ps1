...
foreach ($attachmentName in $expectedAttachmentSchemas.Keys) {
    ...
    try {
        ...
        if ($attachmentName -eq "backupRestore") {
            ...
            if ($proofArtifact.preflight.deploymentShape -ne "passed") {
                $failures += "Deployment evidence manifest attachedEvidence.backupRestore.path artifact preflight.deploymentShape must be passed."
            }
        ...
        if ($attachmentName -eq "rollback") {
            ...
            if ($proofArtifact.preflight.deploymentShape -ne "passed") {
                $failures += "Deployment evidence manifest attachedEvidence.rollback.path artifact preflight.deploymentShape must be passed."
            }
            if ([string]::IsNullOrWhiteSpace($proofArtifact.secretPolicy)) {
