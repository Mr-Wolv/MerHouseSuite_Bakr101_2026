...
When signed Android release, installed Android tour, backup restore drill, rollback rehearsal, or alert-routing proof has already run, pass those generated JSON paths through `-AndroidReleaseManifestPath`, `-InstalledAndroidTourReportPath`, `-BackupRestoreManifestPath`, `-RollbackManifestPath`, and `-AlertRoutingManifestPath`. The deployed evidence manifest validates and records each attached artifact path plus its expected proof schema; Android release `apiBaseUrl`, installed Android tour `apiUrl`, and alert-routing `apiBaseUrl`/`frontendBaseUrl` must match the deployed targets before those items can clear `nextRequiredEvidence`. Alert-routing evidence must also include routed signals, delivery evidence, and a secret-handling policy. The manifest still records `productionClaim=false` until the final live stakeholder walkthrough is complete.
