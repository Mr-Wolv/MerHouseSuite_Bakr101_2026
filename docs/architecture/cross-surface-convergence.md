...
- `frontend/src/pages/NotificationCenterPage.test.tsx`
- `docs/architecture/cross-surface-convergence.md`
...
- `NotificationCenterPage.test.tsx` now proves a failed `Load more alerts` request removes the previous preference-update success message before showing the load-more error.

Proof run:

- `npm --prefix .\frontend run test -- NotificationCenterPage.test.tsx` passed with 10 tests.
...
- `scripts/quality/cross-surface-tour-check.ps1` paired browser/installed-APK report proof.
...
- `docs/development/scripts.md`, `docs/architecture/deployment-ready-local-certification.md`, and `docs/architecture/native-android-local-certification.md` now state that cross-surface report proof validates native PNG screenshot evidence.

Proof run:

- Negative synthetic report proof failed as expected: `.\scripts\quality\cross-surface-tour-check.ps1 -WebReportPath reports\runtime\bh-047\web.json -NativeReportPath reports\runtime\bh-047\native-missing.json` rejected the native report with `Native Android tour report is missing valid PNG screenshot evidence for 1 record(s): PUBLIC /login`.
- Positive synthetic report proof passed: `.\scripts\quality\cross-surface-tour-check.ps1 -WebReportPath reports\runtime\bh-047\web.json -NativeReportPath reports\runtime\bh-047\native-valid.json` checked 100 web records, 80 native records, and 40 normalized role/path pairs.
- PowerShell parser proof passed for `scripts/quality/cross-surface-tour-check.ps1`.
- `.\scripts\quality\markdown-check.ps1` passed.
...
- `NotificationCenterPage.test.tsx` now proves an unread `SERVICE_ACCOUNTABILITY` delivery titled `Service review requested` renders as an action card while ordinary service preference/history text remains review severity.
...
- `npm --prefix .\frontend run test -- NotificationCenterPage.test.tsx` passed with 9 tests.
...
- `.\scripts\quality\frontend-full-tour.ps1 -BaseUrl "http://127.0.0.1:5173" -ApiUrl "http://localhost:8080" -OutputPath ".\reports\bh-036-frontend-full-tour.json"` initially failed.
...
- The full browser tour then passed: `.\scripts\quality\frontend-full-tour.ps1 -BaseUrl "http://127.0.0.1:5173" -ApiUrl "http://localhost:8080" -OutputPath ".\reports\bh-036-frontend-full-tour.json"` passed 5 Playwright tests in 4.6 minutes and wrote a report with 188 checked routes.
- Markdown proof is rerun at close.
...
- `npm --prefix .\frontend test -- src/pages/NotificationCenterPage.test.tsx src/api/client.test.ts --run --no-file-parallelism` passed with 17 tests.
...
- `docs/architecture/deployment-ready-local-certification.md` current scripted certification proof.
...
- README’s current proof bullets also omitted the explicit native API base even though the usage section explained it.
...
- `.\scripts\quality\markdown-check.ps1` passed.
- A targeted PowerShell command-block scan over `README.md`, `docs/architecture/deployment-ready-local-certification.md`, `docs/development/scripts.md`, and `docs/architecture/roadmap.md` passed, proving report-aware `deployment-readiness.ps1` examples that include tour report paths also include `-NativeApiBaseUrl`.
...
- `check.ps1` is the repository’s broad local quality entry point and can be run directly outside the deployment gate.
...
- `scripts/quality/native-android-tour.ps1` route records for active merchant, empty merchant, active warehouse, and empty warehouse users.
- `scripts/quality/cross-surface-tour-check.ps1` role/path and state/path coverage checks.
- `scripts/quality/tour-report-lib.ps1` shared tour-report normalization helpers.
...
- PowerShell parser check passed for `scripts/quality/native-android-tour.ps1`, `tour-report-lib.ps1`, `cross-surface-tour-check.ps1`, and `native-mobile-check.ps1`.
- `.\scripts\quality\native-mobile-check.ps1` passed the structural native Android shell check.
- `.\scripts\quality\mobile-shell-check.ps1` passed.
- `.\scripts\quality\cross-surface-tour-check.ps1` passed against the current paired browser/native reports, proving the strengthened native state/path checks remain compatible with existing evidence while future native reports can carry explicit `stakeholderState`.
- `.\scripts\quality\markdown-check.ps1` passed.
...
- `scripts/quality/native-android-tour.ps1` installed-APK screenshot evidence.
...
- PowerShell parser check passed for `scripts/quality/native-android-tour.ps1`, `tour-report-lib.ps1`, `mobile-shell-check.ps1`, and `native-mobile-check.ps1`.
- `.\scripts\quality\mobile-shell-check.ps1` passed.
- `.\scripts\quality\native-mobile-check.ps1` passed the structural native Android shell check.
- `.\scripts\quality\markdown-check.ps1` passed.
...
- Backend mutation boundaries must match the shared web/native route’s visible read-only service review behavior.
...
- `npm --prefix .\frontend test -- src/pages/NotificationCenterPage.test.tsx src/api/client.test.ts --run --no-file-parallelism` passed with 16 tests.
...
- `frontend/src/pages/NotificationCenterPage.test.tsx` covers the dense-alert case: 50 loaded delivery cards and 73 unread records in the backend summary.

Proof run:

- `npm --prefix .\frontend test -- src/pages/NotificationCenterPage.test.tsx --run --no-file-parallelism` passed with 7 tests.
