...
As of 2026-06-13, V17 has a **confirmed live** private deployed footprint on the selected no-card lane: Neon PostgreSQL, Hugging Face Docker Space backend, Vercel React/Vite frontend, GitHub Actions quality/release workflows, and GitHub Release APK distribution. The deployment infrastructure is complete and verified by CI. The remaining V17 work is **not** deployment infrastructure — it is completing four portfolio features that demonstrate the full user lifecycle: Access Request & Approve-and-Activate, OTP Password Recovery, How To Use Page, and AI Assistant Completion with conversation threading. See [`V17-RE-EVALUATION.md`](../refactor/V17-RE-EVALUATION.md) for the detailed scope, acceptance criteria, and task ordering.
