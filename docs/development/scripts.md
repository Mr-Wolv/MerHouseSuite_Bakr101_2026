...
When sibling V17 proof already exists, pass `-AndroidReleaseManifestPath`, `-InstalledAndroidTourReportPath`, `-BackupRestoreManifestPath`, `-RollbackManifestPath`, and `-AlertRoutingManifestPath`. The wrapper validates that each supplied path exists and is the expected JSON proof artifact schema for that attachment, rather than accepting any schema-shaped file; Android release `apiBaseUrl`, installed Android tour `apiUrl`, and alert-routing `apiBaseUrl`/`frontendBaseUrl` must match the deployed URLs before the proof can clear those evidence items. Alert-routing evidence must also include routed signals, delivery evidence, and a secret-handling policy. It then records the sanitized path, schema, and API target fields under `attachedEvidence`. Attached evidence removes that item from `nextRequiredEvidence`; the final live stakeholder walkthrough still remains an explicit production-claim blocker. The deployment evidence manifest records `productionClaim=false` so smoke proof cannot be mistaken for a completed release.
