...
| `scripts/quality/markdown-check.ps1` | Validate tracked and private markdown links, including Obsidian wiki links, outside generated dependency and report folders. |
...
## Notes For The Future Publication Boundary

The scripts are local development and verification tooling. Runtime values come from `.env`, environment variables, command parameters, or files outside `backend/` and `frontend/`. Generated reports and local runtime files are not part of the future publication boundary. Publication-readiness checks focus on whether `backend/` and `frontend/` can be copied into a separate public repository without carrying sensitive material.
