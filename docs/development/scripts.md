...
When sibling V17 proof already exists, pass `-AndroidReleaseManifestPath`, `-InstalledAndroidTourReportPath`, `-BackupRestoreManifestPath`, and `-RollbackManifestPath`. The wrapper validates that each supplied path exists and is a JSON proof artifact with a non-blank `schema`; Android release `apiBaseUrl` and installed Android tour `apiUrl` must match the deployed `-ApiBaseUrl` before the proof can clear those evidence items. It then records the sanitized path, schema, and API target fields under `attachedEvidence`. Attached evidence removes that item from `nextRequiredEvidence`; alert-routing proof and the final live stakeholder walkthrough still remain explicit production-claim blockers. The deployment evidence manifest records `productionClaim=false` so smoke proof cannot be mistaken for a completed release.
