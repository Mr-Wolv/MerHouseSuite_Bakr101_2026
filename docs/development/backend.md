...
The Docker Compose stack supplies PostgreSQL and the required backend environment variables. Compose builds the backend from `backend/backend-compose.Dockerfile`; the managed Hugging Face deployment image is tracked separately under `deploy/managed/huggingface-backend/Dockerfile`.
